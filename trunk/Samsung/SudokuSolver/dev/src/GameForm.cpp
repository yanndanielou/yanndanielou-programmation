...
GameForm::OnActionPerformed(const Osp::Ui::Control& source, int actionId)
{
		...
		actionId <= ID_FIST_CASE + _casesCount)
	{
		...
		int popupWidth = Application::GetInstance()->GetAppFrame()->GetFrame()->GetWidth() * 0.6;
		int popupHeight = Application::GetInstance()->GetAppFrame()->GetFrame()->GetHeight() * 0.6;
	...
	else if(actionId == ID_RESULT_BUTTON)
	{
		...
		else
		{
			...
			noResultPopup->Construct(false,Dimension(popupWidth, popupHeight));
