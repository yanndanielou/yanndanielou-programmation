...
for aa = 1 : size(nom_var_global,1)
    ...
    if ~isempty(index_remplissage_param)
        ...
        for nb_index_remplissage_param = 1 : size(index_remplissage_param,2)
            index_debut = index_remplissage_param(1,nb_index_remplissage_param);
            nombre_characteres_a_traiter = 0;
            if ~isequal(nb_index_remplissage_param,1)
                index_fin_instruction_param_courant = index_fin_instruction_param(1,nb_index_remplissage_param);
                index_fin_instruction_param_precedent = index_fin_instruction_param(1,nb_index_remplissage_param-1);
                index_fin_instruction_param_suivant = index_fin_instruction_param(1,nb_index_remplissage_param+1);
                
                %CFX00706885: je 
                nombre_characteres_a_traiter = index_fin_instruction_param_courant-index_fin_instruction_param_precedent;
            else
                nombre_characteres_a_traiter = index_fin_instruction_param(1,nb_index_remplissage_param);
            end
            index_fin_dans_chaine_a_traiter = index_debut + nombre_characteres_a_traiter;
            index_dans_chaine_a_traiter = index_debut:index_fin_dans_chaine_a_traiter;
