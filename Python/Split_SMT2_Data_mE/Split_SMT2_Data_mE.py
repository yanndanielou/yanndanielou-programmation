...
def create_and_fill_output_file(output_directory, file_name, file_content_as_list_of_lines):
    LoggerConfig.print_and_log_info('Create output file:' + file_name)
...
def split_SMT2_Data_mE():
    ...
    LoggerConfig.print_and_log_info(input_SMT2_Data_mE_file_name + " has " + str(len(input_SMT2_Data_mE_file_content_as_lines)) + " lines")
    ...
    for line in input_SMT2_Data_mE_file_content_as_lines:
        ...
        if (line_number % 1000_000) == 0:
            LoggerConfig.print_and_log_info("Processing line number:" + str(line_number))
        ...
        if current_step != previous_step:
            LoggerConfig.print_and_log_info("Current step is now:" + current_step)
        ...
        if current_step == part_2_to_99_fill_SMT_mE_aig:
            ...
            if (total_number_of_fill_SMT_mE_aig_lines % Constants.max_number_of_SMT_mE_aig_lines_per_output_files) == 1:
                LoggerConfig.print_and_log_info(str(total_number_of_fill_SMT_mE_aig_lines) + " fill SMT_mE_aig lines, create a new file")
    ...
    LoggerConfig.print_and_log_info("End of parsing input file")

    output_directory_name = "output_" + datetime.datetime.now().strftime("%Y-%m-%d_%Hh%Mmn%Ss")
    LoggerConfig.print_and_log_info("Create output directory " + output_directory_name)
    os.mkdir(output_directory_name)

   
    LoggerConfig.print_and_log_info("Compute output file part 1")
    output_SMT2_Data_mE_part1_initialisation_structures_content_as_list_of_lines.append("")
    ...
    LoggerConfig.print_and_log_info("Compute output files part 2 to 99")
    for output_SMT2_Data_mE_part_2_to_99_fill_SMT_mE_aig_file_contents_as_list_of_lines in output_SMT2_Data_mE_part_2_to_99_fill_SMT_mE_aig_files_contents_as_list_of_lines_per_file:
    ...
    LoggerConfig.print_and_log_info("Compute output file part 100")
    output_SMT2_Data_mE_part100_fill_SMT_mE_feu_BAL_content_as_list_of_lines.append("")
    ...
    LoggerConfig.print_and_log_info("Compute output matlab file to call functions")
