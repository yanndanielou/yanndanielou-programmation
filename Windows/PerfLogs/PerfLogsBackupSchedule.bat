@SETLOCAL
rem @ECHO OFF
@ECHO ** Perflogs : Create %APPLICATION_NAME% backup scheduled task
...
schtasks /delete /tn "%APPLICATION_NAME% Backup" /f > NUL 2>&1
schtasks /create /tn "%APPLICATION_NAME% Backup" /tr "%~dp0%BACKUP_AND_ZIP_BAT%" /ru "System" /sc daily /st %BACKUP_TIME% > NUL
